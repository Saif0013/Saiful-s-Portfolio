    ...
    with torch.no_grad():
        for texts, labels in tq.tqdm(dl, total=len(dl), desc=desc):
            ...
            if sequenceClassificationModel == False:
              ...
              # TODO Task 2b - 2b CHALLENGE 
              #      if you are doing the 2b challenge you should comment out the above line of code
              #      and replace with the following line since your model will already have
              #      the logit removed.
...
def mk_plots(sentence_len, model_fname=None, sequenceClassificationModel = True):
    ...
    ds = datasets.TextDataset('/content/data/txt/val.csv', sentence_len)
    ...
    if sequenceClassificationModel == False:
        ...
        # for 2b challenge load the distilbertmodel 
        # for the other tasks load the AutoModelForSequenceClassification
    ...
    if model_fname is not None:
        ...
        #     (store as pred_masks) 
        embeds, pred_masks = collect_outputs(dl, model, sequenceClassificationModel)
    else:
        model.to(device)
    ...
    embeds, label_masks = collect_outputs(dl, model, sequenceClassificationModel) 
